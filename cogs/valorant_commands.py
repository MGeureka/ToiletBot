...
from services.db.val_database import (add_valorant_username_todb,
                                      remove_valorant_username_fromdb,
                                      update_valorant_username_indb)
from services.api.val_api import check_valorant_username
...
class ValorantCommands(commands.Cog):
    ...
    async def removevalorantprofile(self, interaction: discord.Interaction):
        ...
        try:
            await remove_valorant_username_fromdb(user_id, user_nick)
            logger.info(f"{user_nick} ({user_id}) ran "
    ...
    async def updatevalorantprofile(self, interaction: discord.Interaction,
        ...
        try:
            puuid, region = await check_valorant_username(username, tag)
            await update_valorant_username_indb(username, puuid, tag,
                                               user_nick, user_id)
            logger.info(f"{user_nick} ({user_id}) ran "
