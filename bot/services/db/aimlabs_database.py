...
async def check_dupe_aimlabs_profile_in_guild(db: Database, aimlabs_id: str,
                                              guild_id: int, discord_id: int) -> bool:
    ...
    query = """
            ...
            """
    return await db.fetchval(query, aimlabs_id, guild_id, discord_id)
...
async def add_aimlabs_profile_todb(db: Database, discord_id: int,
    ...
    """Adds an aimlabs profile to the database."""
    now = datetime.now(timezone.utc)
    query = """
...
async def initialize_aimlabs_profile(db: Database,
    ...
    """Adds or updates an aimlabs profile in db. Returns (True, str) if
    process is a relink of an existing inactive profile. (False, None)
    otherwise.
    """
    if await check_dupe_aimlabs_profile_in_guild(db, aimlabs_id, guild_id,
                                                 discord_id):
