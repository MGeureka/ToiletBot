...
async def check_dupe_val_profile_in_guild(
        db: Database, valorant_id: str, guild_id: int, discord_id: int
) -> bool:
    ...
    query = """
            ...
            """
    return await db.fetchval(query, valorant_id, guild_id, discord_id)


async def add_val_account_todb(
        db: Database, valorant_id: str, valorant_username: str,
        valorant_tag: str, region: str
):
...
async def initialize_val_profile(db: Database,
    ...
    otherwise.
    """
    if await check_dupe_val_profile_in_guild(db, valorant_id, guild_id,
                                             discord_id):
        raise UsernameAlreadyExists(f"This account has already been claimed. "
                                    f"Please enter a different username.")
    existing_val_profile = await get_profile_from_db(
