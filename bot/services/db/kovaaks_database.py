...
async def check_dupe_kovaaks_profile_in_guild(
        db: Database, kovaaks_id: str, guild_id: int, discord_id: int
) -> bool:
    ...
    """
    return await db.fetchval(query, kovaaks_id, guild_id, discord_id)


async def add_kovaaks_account_todb(
        db: Database, kovaaks_id: str, kovaaks_username: str, steam_id: str,
        steam_username: str
):
...
async def initialize_kovaaks_profile(db: Database,
    ...
    otherwise.
    """
    if await check_dupe_kovaaks_profile_in_guild(db, kovaaks_id, guild_id,
                                                 discord_id):
